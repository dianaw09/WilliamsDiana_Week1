...
```{r intro}
# Here I am doing some math to demonstrate how R can be used as a calculator.
################### NEW SECTION: NEW PRACTICE 
1+1
2^10
5*6
...
option + command + i will automatically create a chunk of code in my Rmd notebook.

Creating your own variables in R. We use `<-` as an assignment operator to create new variables.
```{r}
x <- 1
x
y = 2 # I would not recommend using the `=` operator to assign variables. ## DO NOT DO THIS. IT'S BAD PRACTICE
y
dog <- "brown"
dog
...
Conditionals - we will practice and implement/apply these a lot later 
```{r}
x > y
y < x
# is y equal to x - this is why i say to avoid using the `=` to assign a value to a variable
x = y #NO 
...
The script is a place to run code so that it can be edited and saved easily for future reference; it mostly only holds commands. The console shows output.
...
'=' assigns value and '==' tests for equality.
HW: Some practice  
...
# for practice - think of another way you could put together this data frame
```
**Question**
...
student.ages
#4
male.ages<-subset(student.ages, gender=="Male")
male.ages
#5
sum(21,19,22)
```
